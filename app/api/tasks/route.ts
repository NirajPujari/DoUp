...
export async function GET(request: Request) {
  try {
    ...
    const tasksRaw = await db
      ...
      .toArray();
    const visibleTasks = tasksRaw
      .filter((task) => isTaskVisibleOnDate(task as Task, targetDate))
      .filter((task) => (typeParam ? task.type === typeParam : true)) as Task[];
    ...
    const sortedTasks = visibleTasks.sort((a: Task, b: Task) => {
      const typeDiff = typeOrder[a.type] - typeOrder[b.type];
      if (typeDiff !== 0) return typeDiff;

      return a.time.localeCompare(b.time); // secondary sort
    });
...
export async function POST(request: Request) {
  try {
    ...
    const { title, description, time, type, date, daysOfWeek, annualDate } =
      taskData;
